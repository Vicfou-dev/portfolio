...
export default function QRLinks({ onClose }) {
  ...
  return (
    ...
    >
      ...
      {/* QR cards */}
      <div className="flex-1 overflow-y-auto flex flex-col justify-center gap-6 px-6 py-8">
        {links.map((link, i) => (
          <motion.div
            key={link.label}
          ...
          >
            ...
            <button
              onClick={() => setExpanded(link)}
              className="flex-shrink-0 rounded-sm overflow-hidden p-2 bg-[#F8F9FA] hover:ring-2 hover:ring-accent/60 transition-all duration-200 cursor-zoom-in"
            ...
            >
              <div className="flex items-center gap-2 mb-1.5 text-muted group-hover/link:text-accent transition-colors">
                {link.icon}
                <span className="text-primary text-sm font-medium">{link.label}</span>
              </div>
              <p className="font-mono text-xs text-muted truncate">{link.mono}</p>
              <p className="font-mono text-[10px] text-muted/50 mt-2 uppercase tracking-widest">Ouvrir ↗</p>
            </a>
          </motion.div>
        ))}
      </div>
